...
import { ShieldAlert, Lock, Mail, AlertTriangle, ArrowRight } from 'lucide-react';
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4 relative bg-slate-950">
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex w-12 h-12 rounded-xl bg-indigo-500/10 border border-indigo-500/20 items-center justify-center text-indigo-400 mb-3">
            <ShieldAlert className="w-6 h-6" />
          </div>
          <h2 className="text-3xl font-extrabold text-white tracking-tight">Admin Console</h2>
          <p className="mt-2 text-sm text-slate-400">Log in with administrative credentials to manage company records.</p>
        </div>
        ...
        <div className="glass-card p-8 rounded-3xl border border-slate-800">
          ...
          <div className="mt-8 pt-6 border-t border-slate-800 text-center text-xs text-slate-500 space-y-2">
            <p>Admin default credentials: <span className="text-slate-300">admin@company.com</span> / <span className="text-slate-300">admin123</span></p>
            <p>Employee looking for portal? <a href="http://localhost:3000" className="text-indigo-400 hover:underline">Access Employee Portal</a></p>
          </div>
        </div>
      </div>
    </div>
