...
AttendanceSchema.pre('save', function() {
  if (this.checkIn && this.checkOut) {
    ...
    this.overtimeHours = this.workingHours > 8 ? Math.round((this.workingHours - 8) * 100) / 100 : 0;
  }
});
...
PayrollSchema.pre('save', function() {
  ...
  this.totalEarnings = Math.round((earn.baseSalary + earn.allowances + earn.overtimePay + earn.bonus) * 100) / 100;
  this.grossPay = this.totalEarnings;
  this.totalDeductions = Math.round((ded.tax + ded.providentFund + ded.insurance + ded.otherDeductions) * 100) / 100;
